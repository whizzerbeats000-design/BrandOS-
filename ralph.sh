...
RALPH_DEFAULT_AGENT="opencode" # set by install CLI; overridden by --agent
...
case "$RALPH_ACTION" in
  print-name)
    if [ "${RALPH_NATIVE_MODE:-0}" = "1" ]; then
      echo "(native mode — no sandbox)"
    else
  ...
  login)
    if [ "${RALPH_NATIVE_MODE:-0}" = "1" ]; then
      echo -e "${Y}Login is only available in Docker/Sandbox mode (sbx not found).${R}"
      exit 0
    fi
  ...
  ports)
    if [ "${RALPH_NATIVE_MODE:-0}" = "1" ]; then
      echo -e "${Y}Ports is only available in Docker/Sandbox mode (sbx not found).${R}"
      exit 0
    fi
...
show_ralph
if [ "${RALPH_NATIVE_MODE:-0}" = "1" ]; then
  echo -e " ${C}Starting Ralph${R} ・ ${Y}v$VERSION${R} ・ Agent: ${Y}$RALPH_AGENT_NAME${R} ・ ${GR}Mode: native${R} ・ Max iterations: ${Y}$MAX_ITERATIONS${R}"
else
...
$(cat "$SCRIPT_DIR/.agent/PROMPT.md")"
  ...
  # In native mode, skip sandbox probing entirely.
  if [ "${RALPH_NATIVE_MODE:-0}" = "1" ]; then
    AGENT_COMMAND="NODE_TLS_REJECT_UNAUTHORIZED=0 opencode run --model "${RALPH_DEFAULT_MODEL:-}" --format json$(format_agent_extra_args) \"\$PROMPT_CONTENT\""
  else
    if sandbox_exists "$RALPH_SANDBOX_NAME"; then
      SANDBOX_EXISTS=1
    else
  ...
  # Timeout: kill agent if it runs longer than this (seconds)
  ...
  while kill -0 "$AGENT_PID" 2>/dev/null; do
    # Check timeout
    AGENT_ELAPSED=$(( $(date +%s) - AGENT_START_TIME ))
    if [ "$AGENT_ELAPSED" -ge "$AGENT_TIMEOUT" ]; then
      echo ""
      echo -e "${YW}⏰ Agent timeout after ${AGENT_TIMEOUT}s — killing process${R}"
