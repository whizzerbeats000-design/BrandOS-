...
# When sbx is missing, sets RALPH_NATIVE_MODE=1 so the main loop runs
# the agent directly instead of inside a Docker sandbox.  Docker mode
# is completely preserved when sbx is present.
check_sbx_available() {
  if ! command -v sbx >/dev/null 2>&1; then
    log_warn "sbx (Docker Sandboxes) not found — running in native mode."
    RALPH_NATIVE_MODE=1
  else
    RALPH_NATIVE_MODE=0
  fi
