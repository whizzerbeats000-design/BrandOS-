...
#   - OpenCode --format json: {"type":"text","part":{"text":"..."}}
#   - Generic: {"type":"text","text":"..."}
parse_json_content() {
  local json_line="$1"

  # Extract text from delta or nested part.text (use jq if available)
  local text=""
  if command -v jq &>/dev/null; then
    text=$(echo "$json_line" | jq -r '.delta.text // .part.text // .text // empty' 2>/dev/null)
  else
    # Greedy grep: find all "text":"..." pairs and take the last one (the content,
    # not the type label).  OpenCode emits {"type":"text","part":{"type":"text",
    # "text":"<content>"}} — the first match is the type label, the second is content.
    text=$(echo "$json_line" | grep -o '"text":"[^"]*"' | tail -1 | LC_ALL=C sed 's/"text":"//;s/"$//' 2>/dev/null)
  fi
  if [ -n "$text" ]; then
...
# Looks for result or step_finish messages containing the final output.
# Supports both Claude-style {"type":"result","result":"..."} and
# OpenCode-style {"type":"step_finish",...} with nested token/cost data.
extract_final_summary() {
  ...
  # 2. OpenCode step_finish — extract token usage as summary
  result_line=$(grep '"type":"step_finish"' "$output_file" 2>/dev/null | tail -1)
  if [ -n "$result_line" ]; then
    if command -v jq &> /dev/null; then
      summary=$(echo "$result_line" | jq -r 'if .part.tokens then "Tokens: \(.part.tokens.total) (in: \(.part.tokens.input), out: \(.part.tokens.output), reasoning: \(.part.tokens.reasoning))" else "" end' 2>/dev/null)
    else
      # grep fallback: extract total tokens from step_finish
      local total=$(echo "$result_line" | grep -o '"total":[0-9]*' | head -1 | LC_ALL=C sed 's/"total"://' 2>/dev/null)
      local input=$(echo "$result_line" | grep -o '"input":[0-9]*' | head -1 | LC_ALL=C sed 's/"input"://' 2>/dev/null)
      local output_t=$(echo "$result_line" | grep -o '"output":[0-9]*' | head -1 | LC_ALL=C sed 's/"output"://' 2>/dev/null)
      if [ -n "$total" ]; then
        summary="Tokens: ${total} (in: ${input:-0}, out: ${output_t:-0})"
      fi
    fi
    if [ -n "$summary" ]; then
      echo "$summary"
